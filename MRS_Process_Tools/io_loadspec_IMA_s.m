...
% If not select information about and extension of MRS DICOM files to be loaded
extDICOM	= '';
if Nfiles_IMA ~= 0 && Nfiles_dcm ~= 0
	error('%s: Directory of MRS DICOM files contains mixed files: Nfiles_IMA = %d \tNfiles_dcm = %d\ndirString = %s\n\n', ...
		sFunctionName, Nfiles_IMA, Nfiles_dcm, dirString);
elseif Nfiles_IMA ~= 0 && Nfiles_dcm == 0
	% Directory only contains .IMA MRS DICOM files
	dirData		= dirData_IMA;
	files		= files_IMA;
	Nfiles		= Nfiles_IMA;
	extDICOM	= '.IMA';
elseif Nfiles_IMA == 0 && Nfiles_dcm ~= 0
	% Directory only contains .dcm MRS DICOM files
	dirData		= dirData_dcm;
	files		= files_dcm;
	Nfiles		= Nfiles_dcm;
	extDICOM	= '.dcm';
...
%% Load all MRS DICOM with specific file extension that are in selected directory
% Indicate progress of loading all MRS DICOM files
h	= waitbar (0, sprintf('Loading MRS DICOM Files with extension %s ...', extDICOM), 'Name', 'Loading MRS DICOM Files');
