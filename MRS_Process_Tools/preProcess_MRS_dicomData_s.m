...
% bPhaseCorrFreqShift = (Optional) Boolean that specifies whether phase correction and
...
if with_water
    ...
    % TODO: add option for ima files, with additional directory
    % FLAG: CHANGES
    % currently determines if file is DICOM 
    if isfolder(filename_w)
        out_w_raw		= io_loadspec_IMA_s(filename_w);
    else
...
switch seqType
	case {'PRESS', 'STEAM', 'sLASER'}
        ...
        % TODO: Not necessary for dicom files, already combined
        %       Add some is_dicom flag to skip over coil combination or
        %       is_combined or something
        % FLAG: CHANGES
        %{
        ...
        %% Temporary move of "raw" .IMA data to coilcombined "cc" data as ima files are already coilcombined
        % FLAG: CHANGES
        % TODO: Make alternative branch if it is_dicom or it is_combined
        % Theory: The later could be determined after loading by checking
        % coil dimensions (should be '0' if no coil combination necessary
        out_cc = out_raw;
        out_noproc = op_averaging(out_cc);
        if with_water
            out_w_cc = out_w_raw;
