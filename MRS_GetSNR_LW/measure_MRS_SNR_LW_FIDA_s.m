...
switch dataFormat_MRS
	...
	case 'dat'
		...
		[data_MRS,data_MRSw,data_MRS_noproc,data_MRSw_noproc,out_w_noproc,out_ref_ECC,out_ref_Quant,out_ref_ECC_noproc,out_ref_Quant_noproc] = ...
			preProcess_MRS_s(...
			dirString,...
			outDirString,...
			seqType_MRS,...
			...
			'RemoveBadAverages', rmbadav,...
			'noStandardDeviation', procParams.nSD,...
			'SpectralRegistrationID', strSpecReg,...
			'DriftCorrection', driftCorr,...
			'Iterations', procParams.iterin,...
			'aaDomain', procParams.aaDomain,...
			'MaxTimeAlignment', procParams.tmaxin,...
			'MaxTimeAlignmentSet', bTmaxset,...
			'medianAlignment', medin,...
			'ppmMinimum_fix', ppmmin_fix,...
			'ppmMaximumArray_fix', ppmmaxarray_fix,...
			'ECC', bECC,...
			'PhaseFrequencyCorrection', bPhaseCorrFreqShift,...
			'MinimizeUserInput', strMinUserIn,...
			'ShowPlots', plotswitch,...
			'GenerateReport', report_switch);
	case 'lcmRAW'
