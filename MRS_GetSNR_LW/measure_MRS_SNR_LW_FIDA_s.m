...
switch dataFormat_MRS
	...
	case 'dat'
		...
		warning('\n\n%s: Input (and output) parameters to routine preProcess_MRS_s(...) might not all be correctly set!\n', sFunctionName);
		[data_MRS,data_MRSw,data_MRS_noproc,data_MRSw_noproc,out_ref_ECC,out_ref_Quant,out_ref_ECC_noproc,out_ref_Quant_noproc] = ...
			preProcess_MRS_s(...
			dirString,...
			outDirString,...
			seqType_MRS,...
			dataType_MRS,...
			'Filename', filename_MRS,...
			'WaterDirectory', dirString,...
			'WaterFilename', filename_w,...
			'OVS', procParams.strOVS,...
			'WaterOVS', procParams.strOVS_w,...
			'Leftshift', procParams.leftshift,...
			'avgBlockSize', procParams.avgBlockSize,...
			'RemoveBadAverages', procParams.rmbadav,...
			'noStandardDeviation', procParams.noSD,...
			'SpectralRegistrationID', procParams.strSpecReg,...
			'DriftCorrection', procParams.driftCorr,...
			'Iterations', procParams.iterin,...
			'aaDomain', procParams.aaDomain,...
			'MaxTimeAlignment', procParams.tmaxin,...
			'MaxTimeAlignmentSet', procParams.bTmaxset,...
			'medianAlignment', procParams.medin,...
			'ppmMinimum_fix', procParams.ppmmin_fix,...
			'ppmMaximumArray_fix', procParams.ppmmaxarray_fix,...
			'ECC', procParams.bECC,...
			'PhaseFrequencyCorrection', procParams.bPhaseCorrFreqShift,...
			'MinimizeUserInput', procParams.strMinUserIn,...
			'ShowPlots', procParams.plotSwitch,...
			'GenerateReport', procParams.reportSwitch);
	case 'lcmRAW'
